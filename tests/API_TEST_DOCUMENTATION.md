...
| Kiosk Identity & Authentication | 5 | Ready |
| Token Signing Service | 5 | Ready |
| Vote Commitment Service | 5 | Ready |
| **Total** | **72** | **Ready** |
...
## 13. Kiosk Identity & Authentication

| Test ID | Description | Endpoint | Method | Payload | Expected Result | Actual Result | Status |
|---------|-------------|----------|--------|---------|-----------------|---------------|--------|
| TC-KI01 | Authenticate voter with fingerprint | /api/v1/kiosk/identity/auth | POST | `{"fingerprint_id": 1, "terminal_id": "esp32-01"}` | Status 200; `{"token": "jwt_token"}` | Pending | - |
| TC-KI02 | Auth with invalid fingerprint | /api/v1/kiosk/identity/auth | POST | `{"fingerprint_id": 999, "terminal_id": "esp32-01"}` | Status 401; "Fingerprint not recognized" | Pending | - |
| TC-KI03 | Auth with missing params | /api/v1/kiosk/identity/auth | POST | `{}` | Status 400; Validation error | Pending | - |
| TC-KI04 | Already voted verification | /api/v1/kiosk/identity/auth | POST | `{"fingerprint_id": 1, "terminal_id": "esp32-01"}` | Status 403; "Already voted or not eligible" | Pending | - |
| TC-KI05 | Election not active check | /api/v1/kiosk/identity/auth | POST | `{"fingerprint_id": 2, "terminal_id": "esp32-02"}` | Status 403; "Election not active" | Pending | - |

---

## 14. Token Signing Service

| Test ID | Description | Endpoint | Method | Payload | Expected Result | Actual Result | Status |
|---------|-------------|----------|--------|---------|-----------------|---------------|--------|
| TC-TS01 | Sign token for voting | /api/v1/kiosk/token/sign | POST | N/A (Headers: Bearer JWT) | Status 200; `{"token": "signed_token"}` | Pending | - |
| TC-TS02 | Sign token without auth | /api/v1/kiosk/token/sign | POST | N/A | Status 401; "Unauthorized" | Pending | - |
| TC-TS03 | Sign already used token | /api/v1/kiosk/token/sign | POST | N/A (Headers: Bearer old_token) | Status 409; "Token already used" | Pending | - |
| TC-TS04 | Multiple signatures | /api/v1/kiosk/token/sign | POST | N/A (Headers: Bearer JWT) | Status 200; New signed token | Pending | - |
| TC-TS05 | Expired auth token signing | /api/v1/kiosk/token/sign | POST | N/A (Headers: Bearer expired_token) | Status 401; "Token expired" | Pending | - |

---

## 15. Vote Commitment Service

| Test ID | Description | Endpoint | Method | Payload | Expected Result | Actual Result | Status |
|---------|-------------|----------|--------|---------|-----------------|---------------|--------|
| TC-VC01 | Commit vote for candidate | /api/v1/kiosk/vote/commit | POST | `{"candidate_id": 1, "election_id": "test_election"}` with Headers: Bearer signed_token | Status 201; `{"tx_hash": "transaction_hash", "status": "confirmed"}` | Pending | - |
| TC-VC02 | Commit vote without auth | /api/v1/kiosk/vote/commit | POST | `{"candidate_id": 1}` | Status 401; "Unauthorized" | Pending | - |
| TC-VC03 | Commit vote with invalid data | /api/v1/kiosk/vote/commit | POST | `{"candidate_id": 999}` with Headers: Bearer token | Status 404; "Candidate not found" | Pending | - |
| TC-VC04 | Duplicate vote attempt | /api/v1/kiosk/vote/commit | POST | Same data with Headers: Bearer token | Status 409; "Vote already cast for this election" | Pending | - |
| TC-VC05 | Commit after election closed | /api/v1/kiosk/vote/commit | POST | Valid data with Headers: Bearer token | Status 403; "Election not active" | Pending | - |

---

## ESP32-Dependent Tests (Updated)

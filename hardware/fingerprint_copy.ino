...
const unsigned long enrollCheckInterval = 4000; // Check for new users every 5 seconds
...
void setup() {
  ...
  delay(500); // CRITICAL: Give sensor half a second to boot up before checking password!
  ...
  if (finger.verifyPassword()) {
    ...
    while (1) { delay(1); } // Halt system if sensor fails
  }
  ...
  Serial.println("\nSystem Ready. Place finger to verify, or use Postman to trigger enrollment.");
}

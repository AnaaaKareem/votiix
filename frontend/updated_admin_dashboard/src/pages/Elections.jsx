...
const stats = [
  { label: 'ACTIVE', count: '12', variant: 'success', icon: Inbox },
  { label: 'REGISTRATION', count: '08', variant: 'accent', icon: UserPlus },
  { label: 'TALLYING', count: '03', variant: 'tally', icon: Calculator },
  { label: 'TOTAL', count: '142', variant: 'neutral', icon: FolderDown },
];
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
            ...
            <Button className="gap-2 bg-slate-900 hover:bg-black shadow-lg shadow-slate-200" onClick={() => navigate('/create-election')}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md group flex items-start justify-between">
            <div>
                <Badge variant={stat.variant} className="mb-4">{stat.label}</Badge>
                <p className="text-4xl font-bold text-slate-900 tracking-tight">{stat.count}</p>
            </div>
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 group-hover:bg-slate-100 transition-colors">
                <stat.icon className="w-6 h-6 text-slate-400 group-hover:text-slate-900 transition-colors" />
            </div>
          </div>
        ))}
      </div>

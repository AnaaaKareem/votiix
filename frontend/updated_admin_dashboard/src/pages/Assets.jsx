...
const Assets = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      {showToast && (
        <div className="fixed top-8 right-8 z-[100] bg-emerald-500 text-white px-6 py-3 rounded-xl shadow-lg shadow-emerald-200/50 flex items-center gap-6 border border-emerald-400/20 animate-in slide-in-from-right-10">
          <div className="flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5 text-emerald-50" />
            <p className="font-semibold text-sm whitespace-nowrap">Candidate Alex Morgan added successfully!</p>
          </div>
          <button onClick={() => setShowToast(false)} className="hover:bg-white/10 p-1 rounded-full transition-colors ml-auto">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
